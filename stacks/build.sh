...
if [[ -d "${IMAGE_DIR}/base" ]]; then
  docker build --platform=linux/amd64 --platform=linux/arm64 -t "${BASE_IMAGE}" "${IMAGE_DIR}/base" --build-arg "stack_id=${STACK_ID}"
fi

# Сборка build-образа
echo "BUILDING ${BUILD_IMAGE}..."
docker build --platform=linux/amd64 --platform=linux/arm64 --build-arg "base_image=${BASE_IMAGE}" -t "${BUILD_IMAGE}" "${IMAGE_DIR}/build"

# Сборка run-образа
echo "BUILDING ${RUN_IMAGE}..."
docker build --platform=linux/amd64 --platform=linux/arm64 --build-arg "base_image=${BASE_IMAGE}" -t "${RUN_IMAGE}" "${IMAGE_DIR}/run"
